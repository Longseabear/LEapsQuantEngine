...
def generate(context: SnapshotContext) -> list[Insight]:
    ...
    for symbol_key in context.symbol_keys:
        ...
        fast_average = _first_value(context, symbol_key, ("sma_5_close", "sma_20_close"))
        ...
        if close is None or fast_average is None or trend_average is None or momentum_3m is None or momentum_6m is None:
        ...
        short_gap = 0.0 if fast_average <= 0 else (close / fast_average) - 1.0
        pullback_bonus = _pullback_bonus(short_gap=short_gap, momentum=composite_momentum, defensive=defensive)
        overheat_penalty = _overheat_penalty(short_gap=short_gap, defensive=defensive)
    ...
    for rank, item in enumerate(selected, start=1):
        ...
        insights.append(
            Insight(
                ...
                weight=weight,
                score=score,
                reason="daa_momentum_pullback_score",
                metadata={
                    "rank": rank,
                    ...
                    "short_gap": item["short_gap"],
                    "pullback_bonus": item["pullback_bonus"],
                    "overheat_penalty": item["overheat_penalty"],
    ...
    for symbol_key in context.symbol_keys:
        ...
        insights.append(
            Insight(
                ...
                weight=0.0,
                score=0.0,
                reason="not_selected_by_daa_pullback_model",
                metadata={
...
def _pullback_bonus(*, short_gap: float, momentum: float, defensive: bool) -> float:
    if defensive or momentum <= 0:
        return 0.0
    if -0.04 <= short_gap <= -0.003:
        return min(0.06, abs(short_gap) * 1.5)
    return 0.0


def _overheat_penalty(*, short_gap: float, defensive: bool) -> float:
    if defensive:
        return 0.0
    if short_gap > 0.035:
        return min(0.08, (short_gap - 0.035) * 1.8)
    return 0.0
