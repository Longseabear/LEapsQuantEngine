...
ALPHA_ID = "semiconduct-kor-volatility-trailing-stop"
VERSION = "0.1.0"
EVALUATION_CADENCE = "every_cycle"
INPUT_RESOLUTION = "daily"
HORIZON = timedelta(days=1)
ATR_MULTIPLIER = 2.6
STDDEV_MULTIPLIER = 2.1
FALLBACK_TRAIL_PCT = 0.09
...
def generate(context: SnapshotContext) -> list[Insight]:
    ...
    for symbol_key in context.symbol_keys:
        ...
        if mark is None or mark["close"] > mark["stop_price"]:
            continue
        ...
        insights.append(
            Insight(
                ...
                magnitude=-severity,
                confidence=min(0.95, 0.70 + severity * 2.0),
                weight=0.0,
                score=severity,
                group_id="krw-semiconductor",
                reason="semiconductor_volatility_trailing_stop_triggered",
                metadata=mark,
            )
        )
    return insights
    ...
    for symbol_key in context.symbol_keys:
        ...
        patches.append(
            StatePatch(
                ...
                reason="semiconductor_trailing_stop_mark",
            )
        )
...
def _trailing_mark(context: SnapshotContext, symbol_key: str) -> dict[str, float | None] | None:
    ...
    high_watermark = max(previous_high or 0.0, close, rolling_high or 0.0)
