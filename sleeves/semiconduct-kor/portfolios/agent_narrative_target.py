...
DEFAULT_TARGET_PATH = "data/operator-targets/semiconduct-kor/latest_target.json"
MODEL_ID = "semiconduct-kor-agent-narrative-portfolio"
STATE_NAMESPACE = "target_artifact"
PORTFOLIO_STATE_SYMBOL = "__portfolio__"


class AgentNarrativeTargetPortfolioModel:
    """Emit morning agent-authored narrative allocation targets."""
    ...
    def __init__(
        ...
        *,
        ...
        self.allowed_markets = tuple(str(market).strip().upper() for market in allowed_markets if str(market).strip())
        self.max_gross_exposure = clamp_pct(max_gross_exposure)
        self.max_position_pct = clamp_pct(max_position_pct)
        self.max_age_hours = max(0.0, float(max_age_hours))
        self.require_sleeve_id = bool(require_sleeve_id)
        self.scale_to_max_gross = bool(scale_to_max_gross)
    ...
    def create_targets(self, context: PortfolioConstructionContext) -> tuple[PortfolioAllocationTarget, ...]:
        ...
        if artifact.flatten:
            targets = tuple(
                PortfolioAllocationTarget(symbol=symbol, target_percent=0.0, tag="agent_narrative_target:flatten")
            ...
            for symbol in context.held_symbols:
                ...
                targets.append(
                    PortfolioAllocationTarget(
                        symbol=symbol,
                        target_percent=0.0,
                        tag="agent_narrative_target:missing_from_daily_artifact",
                    )
                )
    ...
    def state_patches(
        ...
        return (
            StatePatch(
                ...
                value=value,
                reason="agent_narrative_target_artifact_read",
    ...
    def _scaled_items(self, artifact: AgentTargetArtifact) -> tuple[tuple[AgentTargetItem, float], ...]:
        ...
        for item in artifact.targets:
            target_percent = max(-self.max_position_pct, min(self.max_position_pct, item.target_percent * scale))
...
def create_portfolio_model(params: Mapping[str, Any] | None = None) -> AgentNarrativeTargetPortfolioModel:
    values = dict(params or {})
    return AgentNarrativeTargetPortfolioModel(
        ...
        max_gross_exposure=float(values.get("max_gross_exposure", 0.95)),
        max_position_pct=float(values.get("max_position_pct", 0.30)),
...
def _target_tag(item: AgentTargetItem, artifact: AgentTargetArtifact, target_percent: float) -> str:
    ...
    parts = ["agent_narrative_target", f"id={source}", f"w={target_percent:.3f}"]
