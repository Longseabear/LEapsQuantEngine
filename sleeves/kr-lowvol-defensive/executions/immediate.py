...
class LowVolDefensiveExecutionModel:
    def __init__(
        self,
        tag_prefix: str = "kr-lowvol-defensive",
        order_type: str = "limit",
        time_in_force: str = "day",
        limit_offset_bps: float = 6.0,
        max_slice_quantity: int | None = None,
        max_slice_notional: float | None = 1_000_000.0,
        max_slices: int | None = 4,
        urgency: str = "patient",
        max_order_age_seconds: float | None = 1200.0,
        price_drift_bps: float | None = 70.0,
        min_replace_interval_seconds: float | None = 240.0,
...
def create_execution_model(params):
    return LowVolDefensiveExecutionModel(
        tag_prefix=str(params.get("tag_prefix", "kr-lowvol-defensive")),
        order_type=str(params.get("order_type", "limit")),
        time_in_force=str(params.get("time_in_force", "day")),
        limit_offset_bps=float(params.get("limit_offset_bps", 6.0)),
        max_slice_quantity=_optional_int(params.get("max_slice_quantity")),
        max_slice_notional=_optional_float(params.get("max_slice_notional"), default=1_000_000.0),
        max_slices=_optional_int(params.get("max_slices"), default=4),
        urgency=str(params.get("urgency", "patient")),
        max_order_age_seconds=_optional_float(params.get("max_order_age_seconds"), default=1200.0),
        price_drift_bps=_optional_float(params.get("price_drift_bps"), default=70.0),
        min_replace_interval_seconds=_optional_float(params.get("min_replace_interval_seconds"), default=240.0),
        max_replacements=_optional_int(params.get("max_replacements"), default=2),
        allow_sells=_optional_bool(params.get("allow_sells"), default=True),
