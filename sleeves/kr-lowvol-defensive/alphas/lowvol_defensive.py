...
def _is_defensive_reject(item: dict[str, float]) -> bool:
    ...
    if volatility > MAX_NORMALIZED_VOLATILITY and item["momentum_60"] < 0.12:
        return True
    if item["momentum_20"] < -0.06 and item["drawdown_20"] < -0.10:
        return True
    if item["momentum_60"] < -0.10:
        return True
    if item["drawdown_60"] < -0.24 and item["trend"] < 0:
        return True
    if item["gap"] > 0.09 or item["high_low_range"] > 0.12:
        return True
    if item["volume_ratio_20"] >= 4.50:
        return True
    if item["bar_return"] > 0.095 and item["volume_ratio_20"] > 2.0:
        return True
    if item["lottery_penalty"] >= 0.78 or item["crowding_penalty"] >= 0.85:
        return True
    return False
